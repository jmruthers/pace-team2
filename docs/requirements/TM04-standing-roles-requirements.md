...
- Confirm `rbac_app_pages` row for `page_name = 'member-roles'`, `app_id = get_app_id('TEAM')`, `scope_type = 'organisation'` is in place (post-build seeding pass — see §15).

### Domain references

- `pace-core2/packages/core/docs/standards/3-security-rbac-standards.md` — RBAC API conventions; `useResourcePermissions` semantics; `check_rbac_permission_with_context` helper; `get_app_id`. Future cross-app convergence to RBAC-checked RLS for `core_member_role` is informational only — see §17 References.
...
- **MCP test — `rbac_app_pages` row for `member-roles`.** Confirm post-build seeding adds the row for `page_name = 'member-roles'`, `app_id = get_app_id('TEAM')`, `scope_type = 'organisation'` before release.
...
- Post-build RBAC seeding reminder: `rbac_app_pages` must include the row for `page_name = 'member-roles'`, `app_id = get_app_id('TEAM')`, `scope_type = 'organisation'` (post-build seeding pass per architecture).
