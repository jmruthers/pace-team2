...
**Filter execution model.** Server-side filters: `organisation_id = selectedOrganisation.id`, `deleted_at IS NULL`, and per-view `membership_status` (Members: `IN ('Active','Suspended')`; Pending: `= 'Provisional'` joined to `team_member_request`); on the Members tab also `membership_type_id` when the membership-type filter is active. Client-side: free-text search across last name, first name, preferred name, email, and membership number — case-insensitive substring match against the in-memory result set returned by the server query.
...
- **F-05** On entry, the Members tab issues a SELECT against `core_member` joined to `core_person` (and optionally `core_membership_type`), filtered by `organisation_id = selectedOrganisation.id`, `deleted_at IS NULL`, `membership_status IN ('Active','Suspended')`, ordered by `core_person.last_name` asc, `core_person.first_name` asc.
- **F-06** When the Pending tab is opened, the page issues a SELECT against `core_member` joined to `core_person` and to `team_member_request`, filtered by `core_member.organisation_id = selectedOrganisation.id`, `core_member.deleted_at IS NULL`, `core_member.membership_status = 'Provisional'`, and an open `team_member_request` row for the same organisation matching the member with `team_member_request.status IN ('pending','on_hold')` and `team_member_request.request_type IN ('join','transfer')`. Provisional `core_member` rows with no matching open `team_member_request` are not included.
