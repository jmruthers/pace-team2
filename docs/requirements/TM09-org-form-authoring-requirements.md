...
- The page guard uses canonical `pageName='forms'` and `operation='read'`. `rbac_app_pages` must have a row with `page_name='forms'`, `app_id=get_app_id('TEAM')`, `scope_type='organisation'` (post-build seeding noted in §15).
...
- Confirm an `rbac_app_pages` row for `page_name='forms'`, `app_id=get_app_id('TEAM')`, `scope_type='organisation'` is in place (Q-RBAC-1 post-build seeding).
...
- **MCP test — `rbac_app_pages` seeding (Q-RBAC-1 post-build).** Confirm a row exists with `page_name='forms'`, `app_id=get_app_id('TEAM')`, `scope_type='organisation'` via `select * from rbac_app_pages where page_name='forms' and app_id=get_app_id('TEAM');`.
...
- **Implementation blocked until** the following platform-side prerequisites land on dev (`rkytnffgmwnnmewevqgp`):
  ...
  6. **Q-RBAC-1 — `rbac_app_pages` row for `pageName='forms'` under TEAM app.** Post-build seeding before release: `page_name='forms'`, `app_id=get_app_id('TEAM')`, `scope_type='organisation'`. Architecture pre-authorised; without the row, `<PagePermissionGuard pageName="forms">` evaluations always deny.
...
- `pace-core2/packages/core/docs/standards/3-security-rbac-standards.md` — RBAC helper attributes; `check_rbac_permission_with_context`; `get_app_id('TEAM')`; canonical RLS policy templates. **Informational note — future cross-app convergence:** the v6 slice ships against the live `check_user_is_org_admin(organisation_id)` RLS gate on `core_forms` and `core_form_fields` (Q-MUT-1). Future cross-app convergence to RBAC-checked RLS using `check_rbac_permission_with_context('<op>:page.forms', 'forms', organisation_id, NULL, get_app_id('TEAM'))` is informational only and does not gate v1 implementation. (Per cross-app decisions log entry "Mutation contract Option A is conditional, not universal" 2026-05-04, originating from TEAM-03.)
