...
- Unresolved blockers: `5` (`TEAM-02`, `TEAM-05`, `TEAM-09`, `TEAM-11`, `TEAM-13`)
...
| TEAM-02 | TEAM-01 | Blocked | `TM02 §15 gate still fails in live dev DB: required RPC update is incomplete` | Supabase MCP (`project_id=rkytnffgmwnnmewevqgp`) confirms enums are present (`team_member_request_type={member_profile_access,join,transfer}`, `team_member_request_status={pending,approved,rejected,withdrawn,on_hold}`), but `public.app_update_member_request_status` is absent from `pg_proc`; keep blocked until this RPC is deployed |
...
- Strict Supabase MCP unblock pass complete. Remaining blocked slices are now evidence-specific: `TEAM-02`/`TEAM-05` (`app_update_member_request_status` missing), `TEAM-09` (`core_field_list` `member_number` seed missing), `TEAM-11` (report-domain reseed incomplete), `TEAM-13` (smoke-send evidence missing).
